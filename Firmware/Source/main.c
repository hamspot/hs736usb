...
char psn = 0;
char cmd[10];
char freqbuffer[5] = { 0x14, 0x50, 0x00, 0x00, 0x08};
char freqbufferRX[5] = { 0x14, 0x50, 0x00, 0x00, 0x00};
char freqbufferTX[5] = { 0x44, 0x00, 0x00, 0x00, 0x00};
char RXstatusbuffer[5] = { 0x80, 0x80, 0x80, 0x80, 0xE7};  
char TXstatusbuffer[5] = { 0x80, 0x80, 0x80, 0x80, 0xF7}; 
char Tone[38] = { 0x1E,0X20,0X22,0X24,0X26,0X28,0X2A,0X2C,0X2E,0X30,0X32,0X34,0X36,0X38,0X16,0X1A,0X1F,
                  0X21,0X23,0X25,0X27,0X29,0X2B,0X2D,0X2F,0X31,0X33,0X35,0X37,0X39,0X18,0X1C,0X13,0X15,
						0X17,0X19,0X1B,0X1D };
char x = 0;
...
SPBRG = 0xC3; //was0x70    0xC3 = 4800 baud rate
SPBRGH = 0x09; // 0x0271 for 48MHz -> 19200 baud was 0x02 chg to 9600 baud 0x4E2, 0x09 = 4800 baud rate
 ...
 * Side Effects:    None
 *
 * Overview:        This routine should send what is in the Cmd array to the serial line(736)
 *                  
 *
 * Note:            
 *
...
{ 
/**************************************************************************** 
new code  12/27/10 added by chuck jones (c) 2010
    This is code added to Microchip's proprietary USB framework for a 18f14k50
	 PIC microcontroller to control a Yaesu FT-736R Transceiver via Ham Radio Deluxe
    Copyright (C) 2010  Albert C. Jones
...
******************************************************************************/
for(x = 0; x < LastRS232Out;  ++x)				// collect bytes from HRD commands
{	
	cmd[psn] = RS232_Out_Data[x];
	++psn;
}
if (psn > 4)
{
if (cmd[4] == 0xE7)                  // Receive status
{	
	while( !USBUSARTIsTxTrfReady());
	putUSBUSART(RXstatusbuffer,5);
}		
	else if (cmd[4] == 0xF7)                  // Transmit status
{	
	while( !USBUSARTIsTxTrfReady());
	putUSBUSART(TXstatusbuffer,5);
}	
		else if (cmd[4] == 0x03)                  // freq read by HRD
{	
	while( !USBUSARTIsTxTrfReady());
	putUSBUSART(freqbuffer,5);
}			
		else if (cmd[4] == 0x00)                     	// CAT on
{	
	To736();
	for(x = 0; x < 4;  ++x)
	cmd[x] = freqbuffer[x];							// set freq to 145.0000 to synchronize both
	cmd[4] = 0x01;
	To736();
	cmd[0] = 0x08;
	cmd[4] = 0x07;
	To736();												// Set mode to FM also to synchronize
}	
	else if (cmd[4] == 0x80)                  // CAT off
{
	To736();	
}
	else if (cmd[4] == 0x01)                  // Freq set in 736 and read into freq_buffer
{
	for(x=0; x <4; ++x)								// Send freq to 736 and save freq into buffer
		freqbuffer[x] = cmd[x];
	if ((cmd[0] > 0x23) && (cmd[0] < 0x31))
		cmd[0] = cmd[0] + 0xA0;						// allow 1.2 ghz to work, add 1 ghz to 736 only not buffer
	To736();
}	
	else if (cmd[4] == 0x11)                  // Sub RX Freq set and read into freqbufferRX
{
	for(x=0; x <4; ++x)								// Send freq to 736 and save freq into freqbufferRX
		freqbufferRX[x] = cmd[x];
	cmd[4] = 0x1E;
	if ((cmd[0] > 0x23) && (cmd[0] < 0x31))
		cmd[0] = cmd[0] + 0xA0;						// allow 1.2 ghz to work, add 1 ghz to 736 only not buffer
	To736();
}		
		else if (cmd[4] == 0x21)               // Sub TX Freq set and read into freqbufferTX
{
	for(x=0; x <4; ++x)								// Send freq to 736 and save freq into freqbufferTX
		freqbufferTX[x] = cmd[x];
	cmd[4] = 0x2E;
	if ((cmd[0] > 0x23) && (cmd[0] < 0x31))
		cmd[0] = cmd[0] + 0xA0;						// allow 1.2 ghz to work, add 1 ghz to 736 only not buffer
	To736();
	...
	else if (cmd[4] == 0x13)                  // freq read by HRD SAT RX
{	
	while( !USBUSARTIsTxTrfReady());
	putUSBUSART(freqbufferRX,5);
}	
	else if (cmd[4] == 0x23)                  // freq read by HRD  SAT TX
{	
	while( !USBUSARTIsTxTrfReady());
	putUSBUSART(freqbufferTX,5);
}	
	else if (cmd[4] == 0x09)              		// Split direction
{
	cmd[4] = cmd[0];
	To736();
}
	else if (cmd[4] == (0x0B))    				// CTCSS tone code
{
	cmd[4] = 0xFA;										// Reads 847 tone and indexes into Tone array
	if (cmd[0] > 31)									//skips missing 847 codes
	x = cmd[0] - 26;
	else x =cmd[0];
	cmd[0] = Tone[x];
	To736();
}   
	else if (cmd[4] == (0x0A))  					// CTCSS Status set
{	
	cmd[4] = cmd[0];
	if (cmd[0] == 0x2A)
	cmd[4] = 0x0A;
	...
	else if (cmd[4] == (0x07))                  // Mode set
{	
	To736();
	freqbuffer[4] = cmd[0];	
}	
	
	else if (cmd[4] == 0x17)                  // Sub RX mode set
{	To736();
	freqbufferRX[4] = cmd[0];						// save mode into freqbufferRX
}	
	
	else if (cmd[4] == 0x27)                  // Sub TX mode set
{	To736();
	freqbufferTX[4] = cmd[0];						// save mode into freqbufferTX
}	
	
	else if (cmd[4] == 0x08)                   // Transmit 
{
	To736();
	for(x=0; x <4; ++x)	
{	
		RXstatusbuffer[x] = (0x40);				// send dummy info to hrd
		TXstatusbuffer[x] = (0x00);
}
}
	else if (cmd[4] == 0x88)                   // Receive 
{
	To736();
	for(x=0; x <4; ++x)	
{	
		RXstatusbuffer[x] = (0x47);				// send dummy info to hrd
		TXstatusbuffer[x] = (0x80);
}
}
	
	else if (cmd[4] == 0x4E)                   // Satellite mode on
{	
	cmd[4] = 0x0E;
	To736();
	
//	for(x = 0; x < 4;  ++x)
//	cmd[x] = freqbufferRX[x];						 // set freq to 145.0000
//	cmd[4] = 0x1E;
//	To736();
}
	else if (cmd[4] == 0x8E) 						// Satellite mode off
{
	To736();
	for(x = 0; x < 4;  ++x)
	cmd[x] = freqbuffer[x];							// set freq to 145.0000
	cmd[4] = 0x01;
	To736();
} 
}   
if (psn > 5)                                  //  if more than 5 chars received rotate them into next cmd
	for (x = 0; x < (psn - 5); ++x)
	cmd[x] = cmd[x+5];
if (psn > 4)
psn = psn - 5;
//  end new code  by chuck jones          
